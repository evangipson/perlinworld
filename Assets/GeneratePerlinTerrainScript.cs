...
public class GeneratePerlinTerrainScript : MonoBehaviour {
    ...
    {
        // set our frequency to be based off the map size
        frequency = ((height / 100) + (width / 100)) / 2 > 4.0f ? ((height / 100) + (width / 100)) / 2 : 4.0f;
    ...
    {
        // how much is each layer going to effect the final map?
        float persistence = 0.5f;
        float noise = 0f;
        // used for clamping final resultz
        float maxValue = 0;
